...
## [0.2.0](https://github.com/VijayakumarRavi/dumper/compare/v0.1.0...v0.2.0) - 2026-09-25

### Added

- *(database)* support PostgreSQL extensions and bump version to v0.2.0
- *(database)* support libpq and MySQL key-value connection strings
...
- *(ci)* use cgr.dev/chainguard/minio for S3 testing in real-e2e job
- *(ci)* start MinIO using standalone binary in real-e2e job
- *(ci)* upgrade rustls to 0.23.45 for RUSTSEC-2026-0285 and use quay.io MinIO mirror
- *(postgres)* rename dbe variable to db_err to resolve typos linter failure
- *(postgres)* topologically sort views by dependency order
- *(mysql)* use column metadata to distinguish binary vs text types
- *(postgres)* propagate catalog query errors instead of silent unwrap_or_default
- *(mysql)* normalize SSL mode connection URL parameters
- trust webpki root certs in reqwest, prevent silent mysql truncation, and include tag in retention grouping
- *(ci)* sort Cargo.toml dependencies and use dead PIDs for temp file cleanup tests
- *(repository)* protect in-flight temp files, increase snapshot entropy, and fix SigV4 path encoding
- *(stream)* bound frame payload sizes and expand decoder limits
- *(repository)* ensure lock cleanup on error, fix windows write and path traversal
- *(postgres)* sanitize identifiers and fix transaction isolation in backup and restore
- *(mysql)* sanitize identifiers and handle routine conflicts in backup and restore
...
- *(stream)* align fuzz oversized payload test with 32 MiB max payload limit
...
- *(crypto)* zeroize cryptographic keys and master keys in memory
- *(cli)* mask all database URL secrets and suppress password echo
